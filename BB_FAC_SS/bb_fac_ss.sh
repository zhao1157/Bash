...
ener_file=fe17a_K_shell.en	#the original energy file
tr_file=fe17a_K_shell_test.tr 	#the original tr file


tr_dir=$( echo ${tr_file/'.tr'/} )	#the output directory
...
for J2_up in $(seq 0 2 ${J_MAX}) 	#2j must be even, so the increment is 2
do
	for PI_up in 0 1
	do
				...
				do
					...
					then
						...
						then
							...
							then
								echo line $j: lower state has non-negative energy >> ${tr_dir}/out_record 
							fi
